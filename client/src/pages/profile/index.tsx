...
const Profile = () => {
  ...
  function saveChanges(event: MouseEvent<HTMLButtonElement, MouseEvent>): void {
    throw new Error("Function not implemented.");
  }

  return (
    <div className="bg-[#1b1c24] h-[100vh]  flex items-ecnter justify-center flex-col gap-10">
      <div className="flex flex-col gap-10  w-[80vw] md:w-max">
        ...
        <div className="grid grid-cols-2">
          ...
          >
            <Avatar className="h-32 w-32 md:w-48 rounded-full overflow-hidden">
              {userData.image ? (
                ...
                >
                  {userData.firstname
                    ? userData.firstname.split("").shift()
          ...
          <div className="flex min-w-32 md:min-w-64 flex-col test-white items-center justify-center">
              ...
              {colors.map((color, idx) => (
                <div
                  className={`${color} h-8 w-8 rounded-full cursor-pointer transition-all duration-300 ${
                    selectedColor === idx
                      ? "outline-white/50 outline outline-1"
                      : ""
                  }`}
                  key={idx}
